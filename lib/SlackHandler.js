...
function SlackHandler (options) {
    ...
    self.projects = JSON.parse(self.projects);
}
...
SlackHandler.prototype.renderMessage = function (event, callback) {
    ...
    const getDeploymentInfo = function getDeploymentInfo (appId, callback) {
        const onDeploymentsLoaded = function onDeploymentsLoaded (response) {
            let responseData = "";
            ...
            response.on("end", function () {
                ...
                try {
                    ...
                    return callback(null, parsed.app.instances);
                }
                catch (e) {
        ...
        http.get("http://master.mesos:8080/v2/apps/" + appId, onDeploymentsLoaded);
    };
    ...
    function parseEvents (event) {
        ...
        switch (event.type) {
            ...
            case "deployment_success":               
                let text = "The deployment `" + event.data.id + "` was completed successfully at " + prepareDateToken(event.data.timestamp);
                text += "\nApps affected:";    
                let loadingScalingInfo = false;            
                event.data.plan.steps.forEach(function (step) {
                    if (loadingScalingInfo) {
                        return;
                    }
                    ...
                    step.actions.forEach(function (action) {
                        ...
                        console.log("known projects:" + JSON.stringify(self.projects));
                        ...
                        switch (action.action) {
                            ...
                            case "ScaleApplication": {
                                loadingScalingInfo = true;
                                return getDeploymentInfo(action.app, function (error, result) { 
                                    if (!error) {
                                        console.log("Updating " + action.app + " to " + result + " instances");
                                        text += " was scaled to " + result + " instances";
                                    }
...
SlackHandler.prototype.sendMessage = function (message) {
    ...
    let self = this;
    console.log("num projects tagged for message:" + message.projects.length);
